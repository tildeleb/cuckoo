...
func trials(tables, buckets, slots, trials int) float64 {
	tot := float64(0)
	for t := 0; t < trials; t++ {
		...
		for i := base+1; i <= base + tables * buckets * slots; i++ {
			if !ck.Insert(Key(i), Value(uint32(cnt))) {
				...
				fmt.Printf("failed: bumps=%d, %d/%d=%0.2f\n", ck.bumps, ck.inserts, ck.elements, percent)
				for k, v := range ck.stats {
					fmt.Printf("table[%d]: %d/%d=%0.2f\n", k, v.elements, v.size, float64(v.elements)/float64(v.size))
				}
				tot += percent
				maxi = i
				break
...
func main () {
    ...
    if *auto {
		...
		avg := trials(*ntables, *nbuckets, *nslots, *ntrials)
		fmt.Printf("tables=%d, buckets=%d, slots=%d, trials=%d, avg=%0.2f\n", *ntables, *nbuckets, *nslots, *ntrials, avg)
	}
}
