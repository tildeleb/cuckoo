...
func benchmark64(n int) {
	...
	fmt.Printf("benchmark64: gen n=%d\n", n)
	start := time.Now()
	for i := 0; i < n; i++ {
		u[0] = uint64(i)
...
func benchmark32(n int) {
	...
	fmt.Printf("benchmark32: gen n=%d\n", n)
	start := time.Now()
	for i := 0; i < n; i++ {
		u[0] = uint32(i)
		h := jenkins3.HashWords(u, 1, 0)
		hashes[i] = h
		//fmt.Printf("i=%d, 0x%08x, h=0x%08x\n", i, i, h)
	}
	stop := time.Now()
...
func main() {
	...
	u := stu(q)
	h1 := jenkins3.HashWords(u, (len(q)-1)/4, 13)
	h2 := jenkins3.HashWords(u, (len(q)-5)/4, 13)
	h3 := jenkins3.HashWords(u, (len(q)-9)/4, 13)
